...
To run a child workflow independently without blocking the parent, use [`start()`](/docs/api-reference/workflow-api/start) directly in your workflow function. This launches the child workflow in the background.
...
With background execution, the parent workflow continues immediately after starting the child. The child workflow runs independently with its own event log and can be monitored separately using the returned `Run` object.
